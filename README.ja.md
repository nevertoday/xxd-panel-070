...
検証可能なオリジナル公開作例と出典が未提供のため、`sample-01`–`sample-04` は掲載していません。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 070 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

一般的な写真のイラスト化は、場面全体を描いて窮屈になるか、汎用的な漫画調に置き換えて被写体の同一性と色彩精神を失いがちです。

**Panel 070** は、写真から最も記憶に残る被写体、姿勢、関係、比喩だけを残し、手描きの輪郭線と明るい厚塗り／半透明の色面で再編集します。ミニチュアまたは中程度の被写体を大きな暖白の余白に置き、少量のグリッド、飛沫、光、幾何学記号を編集的な添え物にします。細いタイプライター風の文字は後から重ねるのではなく、画像と一緒に構図を作ります。

### 向いている用途

- 被写体を一目で認識できるまま、写真を編集的な絵画へ変換したい場合
- 濁ったフィルターや安価なキャンディ色ではなく、明るく生命感のある色を求める場合
- ミニチュア尺度、明るい余白、方向の流れ、少量の文字を一つのリズムにしたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 070 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 手描き輪郭 · 明るい厚塗り／半透明色面 · ミニチュア主体 · 暖白の余白 · タイプライター風編集書体
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 明るく癒やしがあり、軽やかで出版秩序を備えた手描き編集画面 |
| **識別しやすい特徴** | 暖白の余白に浮かぶミニチュア主体、手描き輪郭と鮮やかな厚塗り／半透明色面、抑制されたタイプライター風の小字 |
...
```text
同一性、姿勢、関係を固定 → 完全な場面と不要背景を削除 → 被写体を誌面に適した尺度へ調整 → 手描き輪郭と明るい厚塗り／半透明色面で再構成 → 元写真の最も鮮活な色を再調整 → 少量のグリッド、飛沫、光だけを追加 → 暖白の余白に抑制的な文字を編成
```

## 完成作の特徴

- 場面全体を描き直さず、最も記憶に残る同一性、姿勢、輪郭、物語関係だけを保つ。
- 輪郭は明確でも手仕事を残し、色面にはガッシュ、水彩、パステル、少量の油彩感を交差させる。
- ミニチュアまたは中程度の被写体を暖白の大きな余白に置き、主従、方向、図文関係で誌面を作る。
- グリッド、飛沫、光、色点、小さな幾何学記号は少量の編集的補助に限定する。
- 色は外部パレットを課さず、元写真の最も明るく生命力ある部分から再調整する。
- 細くわずかに印字ずれのあるタイプライター風文字を、写真の被写体、場所、感情、比喩から生成する。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-070-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 070 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-070 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-070 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 070 を適用し、共通設定は一度だけ解決
/xxd-panel-070 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-070`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-070-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-070-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

70 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 070 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
