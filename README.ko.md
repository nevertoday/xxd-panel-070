...
# 🦁 XXD Panel 070｜임파스토 힐링 섬

### 사진을 밝고 가벼우며 출판 감각이 있는 손그림 편집 화면으로 압축합니다
...
검증 가능한 원본 공개 예시와 출처가 없어 `sample-01`–`sample-04`는 현재 표시하지 않습니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 070이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

일반적인 사진 일러스트는 장면 전체를 묘사해 답답해지거나, 범용 만화풍을 적용해 피사체의 정체성과 원본의 색채 정신을 잃기 쉽습니다.

**Panel 070**는 사진에서 가장 기억할 만한 피사체, 자세, 관계, 은유만 남기고 손그림 윤곽과 밝은 임파스토／반투명 색면으로 다시 편집합니다. 미니어처 또는 중간 크기의 피사체를 넓은 웜 화이트 여백에 두고, 소량의 그리드, 분사, 빛무리, 기하학 표식을 편집적 보조로 사용합니다. 가는 타자기풍 글자는 완성물 위에 덧붙이는 대신 이미지와 함께 구도를 만듭니다.

### 이런 경우에 적합합니다

- 인물·건축·동물·식물·사물·풍경의 정체성을 유지한 채 편집 회화로 바꾸고 싶을 때
- 탁한 필터나 값싼 사탕색이 아닌 밝고 생명력 있는 색을 원할 때
- 미니어처 스케일, 밝은 여백, 방향 흐름, 적은 글자를 하나의 리듬으로 구성할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 070로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 손그림 윤곽 · 밝은 임파스토／반투명 색면 · 미니어처 피사체 · 웜 화이트 여백 · 타자기풍 편집 서체
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 밝고 치유적이며 가볍고 출판 질서가 분명한 손그림 편집 화면 |
| **대표 특징** | 웜 화이트 여백의 미니어처 피사체, 손그림 윤곽과 생생한 임파스토／반투명 색면, 절제된 타자기풍 작은 글자 |
...
```text
정체성·자세·관계 고정 → 완전한 장면과 불필요한 배경 삭제 → 피사체를 편집 스케일로 조절 → 손그림 윤곽과 밝은 임파스토／반투명 색면으로 재구성 → 원본의 가장 생생한 색 재조율 → 소량의 그리드·분사·빛만 추가 → 웜 화이트 여백에 절제된 문구 편집
```

## 완성작의 특징

- 전체 장면을 다시 그리지 않고 가장 기억할 만한 정체성, 자세, 윤곽, 서사 관계만 보존합니다.
- 윤곽은 명확하되 수작업 오차를 남기고 색면에는 과슈, 수채, 파스텔, 소량의 유화 질감을 섞습니다.
- 미니어처 또는 중간 크기 피사체를 넓은 웜 화이트 여백에 두고 위계, 방향, 이미지와 글자의 관계로 편집면을 만듭니다.
- 그리드, 분사, 빛무리, 색점, 작은 기하학 표식은 소량의 편집적 보조로만 사용합니다.
- 외부 팔레트를 강제하지 않고 원본의 가장 밝고 생명력 있는 부분에서 색을 다시 조율합니다.
- 사진의 피사체, 장소, 감정, 은유에서 가늘고 미세하게 어긋난 타자기풍 문구를 생성합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-070-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 070의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-070 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-070 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-070 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-070 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 070 적용, 공통 설정은 한 번만 확정
/xxd-panel-070 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-070`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-070-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-070-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

70개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 070까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
