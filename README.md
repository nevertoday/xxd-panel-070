...
原始 X 样张及其可核验出处尚未提供，因此 `sample-01`–`sample-04` 暂不展示。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 070 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

很多照片转成插画后，要么完整描摹而显得拥挤，要么套用通用卡通模板，失去主体身份和照片本来的色彩精神。

**Panel 070** 从照片中只留下最值得记住的主体、姿态、关系与隐喻，再用手绘描边、明亮厚涂或半透明色块重新组织。微缩或中等尺度的主体进入大面积暖白留白，少量网格、喷点、光晕与几何标记承担编辑性陪衬；精细的打字机式小字则与图像共同构图，而不是覆盖在成品上。

### 适合这些情况

- **主体必须可认：** 人物、建筑、动物、植物、器物或风景的身份不能被通用色斑替换。
- **想要鲜活而不廉价：** 从照片最明亮、最有生命力的颜色重新调制，不做灰脏滤镜或糖果色堆叠。
- **需要专业版式：** 微缩主体、明亮留白、方向引导与克制小字共同决定视觉节奏。
...
> 用 XXD Panel 070 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 手绘描边 · 明亮厚涂 · 半透明色块 · 微缩主体 · 暖白留白 · 打字机式编辑字体

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 070 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 把照片重构成明亮、治愈、轻盈而具有出版秩序的手绘编辑画面 |
| **一眼可辨的特点** | 微缩主体悬置于暖白留白，手绘描边与鲜活厚涂／半透明色块交织，配以克制的打字机式小字 |
...
```text
锁定主体身份、姿态与关系 → 删除完整场景和次要背景 → 将主体缩至适合版面的尺度 → 以手绘描边和明亮厚涂／半透明色块重构 → 从原图最鲜活的颜色重新调制 → 用少量网格、喷点或光晕建立编辑节奏 → 在暖白留白中完成克制的图文编排
```

## 成品中最容易识别的风格特点

- 主体只保留最值得记住的身份、姿态、轮廓与叙事关系，不完整复刻复杂场景。
- 手绘描边清晰但不机械，保留轻微手作误差；色块可交织水粉、水彩、粉彩与少量油画触感。
- 主体以微缩或中等尺度安放在大量暖白留白中，依靠主次、方向和图文关系建立版式。
- 网格、喷点、光晕、色点或小几何标记只作少量编辑性陪衬，不形成第二叙事中心。
- 色彩从原图最明亮、最鲜活、最有生命力的部分重新调制，不采用外层固定色板。
- 文字使用纤细、略带印字误差的打字机式编辑字体，从当前照片的主体、地点、情绪或隐喻中生成。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-070-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 070 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-070 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-070 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 070；公共设置只解析一次
/xxd-panel-070 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-070`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-070-prompt.en.md) · [中文运行适配器](references/xxd-panel-070-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

70 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–070 的全部项目地址和核心风格特点；当前项目以粗体标出。
