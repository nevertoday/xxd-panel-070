...
Many photo-to-illustration workflows either trace the whole scene until it feels crowded or apply a generic cartoon treatment that loses the subject's identity and the photograph's colour spirit.

**Panel 070** retains only the source's most memorable subject, pose, relationship, and metaphor, then reorganises them with hand-drawn contours and bright impasto or translucent colour blocks. A miniature or medium-scale subject sits within generous warm-white space; sparse grids, spray marks, glows, and geometric notations provide editorial support, while fine typewriter-like text composes with the image instead of sitting on top of it.
...
- editorial layouts where miniature scale, bright space, directional flow, and sparse type share one rhythm;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 070 on this image and recommend the most suitable composition and size first.
...
**Signature:** hand-drawn contours · bright impasto／translucent colour · miniature subject · warm-white space · typewriter-like editorial type
...
|---|---|
| **What you get** | A bright, healing, light hand-drawn artwork with clear publication order |
| **Recognisable signature** | A miniature subject held in warm-white space, with hand-drawn contours, vivid impasto or translucent colour, and restrained typewriter-like text |
...
```text
lock identity, pose, and relationship → remove the complete scene and secondary background → set the subject at an editorial scale → reconstruct with hand-drawn contours and bright impasto／translucent colour → remix the source's most vital colours → add only sparse grids, spray, or glow → compose restrained type in warm-white space
```
...
- Keep contours clear but handmade; mingle gouache, watercolour, pastel, and selected oil-paint texture in the colour areas.
- Hold a miniature or medium-scale subject within generous warm-white space, using hierarchy, direction, and image–text relations to build the page.
- Use grids, spray, glow, colour dots, or geometric notations only as sparse editorial support.
- Remix colour from the source's brightest and most vital areas rather than imposing an outer palette.
- Generate fine, slightly misregistered typewriter-like copy from the current photograph's subject, place, mood, or metaphor.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-070-prompt.en.md)
...
```text
/xxd-panel-070 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-070 photo.jpg --prefs last
/xxd-panel-070 photo.jpg --prefs edit
/xxd-panel-070 photo.jpg --prefs new
```
...
/xxd-panel-070 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-070 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-070 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-070 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-070 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-070 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-070 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-070 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-070 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-070 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 070 to a whole directory; shared settings resolve once
/xxd-panel-070 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-070 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-070.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-070" ~/.codex/skills/xxd-panel-070
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-070 --skill xxd-panel-070
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-070`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-070-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-070-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

Each of the 70 Panels preserves its own original brief and aesthetic logic. The table continuously lists every project from 001 through 070 with its defining style characteristics; the current project is bold.
...
| **Soldiers** | `xxd-panel-NNN` (currently 001–070) | Each numbered Skill executes only its own original brief and aesthetic, completing the individual job assigned by the General. |
